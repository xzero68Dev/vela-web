...
type Product = {
  ...
  price: number; price_shopee: number; stock: number; active: boolean; sort_order: number
}
...
export default function ProductPage() {
  ...
  const shopeeP    = current?.price_shopee || 0
  const discount   = shopeeP > price ? Math.round((1 - price / shopeeP) * 100) : 0
            ...
            <div className="flex items-end gap-3 mb-5">
              ...
              {shopeeP > price && (
                <div className="flex items-center gap-1.5 mb-1">
                  <span className="text-sm line-through opacity-40" style={{ color: textColor }}>฿{shopeeP}</span>
                  <span className="text-xs font-mono px-2 py-0.5 rounded-full"
                    style={{ background: meta.accent + '30', color: meta.accent }}>-{discount}%</span>
                </div>
              )}
            </div>
                ...
                return (
                  <Link key={p.sku} href={`/product/${p.sku}`}
                    ...
                    <div className="px-3 pb-3">
                      ...
                      <p className="font-mono text-xs" style={{ color: m.dark ? '#C5BAB0' : '#8C7B6E' }}>฿{p.price}</p>
                    </div>
                  </Link>
                )
              })}
